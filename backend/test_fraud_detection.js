...
async function runFraudTests() {
  ...
  try {
    ...
    const reqRes = await client.post(
      "/requests",
      {
    ...
    console.log(
      `   Captured Fingerprint?? (Check DB manually if fail): We sent ${buyerFingerprint}`,
    );
    ...
    try {
      await client.post(
        ...
        { headers: { "x-device-fingerprint": buyerFingerprint } },
      ); // SAME FINGERPRINT
      ...
      if (error.response && error.response.status === 403) {
        ...
        console.log(
          "❌ FAIL: Rejected but wrong status:",
