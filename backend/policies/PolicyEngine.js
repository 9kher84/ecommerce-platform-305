...
class PolicyEngine {
  ...
  static allows(user, resource, resourceType, action = "view") {
    ...
    if (!policy) {
      console.warn(
        `PolicyEngine: No policy found for resource type '${resourceType}'`,
      );
  ...
  static trace(user, resource, resourceType, action) {
    ...
    const classify = (field, value, expectedType = "string") => {
      ...
      if (
        strVal.includes("'--") ||
        strVal.includes("<script>") ||
        strVal.includes("UNION SELECT")
      ) {
        violations.push({
          field,
          expectedType,
    ...
    const ownerId =
      process.env.OWNER_ID || "11111111-1111-1111-1111-111111111111";
        ...
        policy.rules.forEach((rule, idx) => {
          ...
          if (relevant) {
            timeline.push({
              index: ruleIndex++,
              ruleId: rule.id,
              ruleType: rule.actions.includes("*")
                ? "CONSTRAINT"
                : "PERMISSION",
              priority: 1000 + idx,
              evaluated: false,
    ...
    if (policy.rules) {
      ...
      for (const rule of constraints) {
        ...
        timeline.push({
          ...
          inputs: {
            ...
            "resource.city": classify(
              "resource.city",
              resource ? resource.cityId : null,
              "any",
            ),
          },
        });
      ...
      // B. Permissions
      const permissions = policy.rules.filter(
        (r) => r.actions.includes(action) && !r.actions.includes("*"),
      );
