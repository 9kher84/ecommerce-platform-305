...
const rules = [
  ...
  },
  {
    ...
    actions: ["suspend"],
    check: (user, resource) =>
      resource && user.context && user.context.cityId === resource.cityId,
  },
  {
...
const RequestPolicy = (user, resource, action) => {
  ...
  if (
    user.id === (process.env.OWNER_ID || "11111111-1111-1111-1111-111111111111")
  ) {
    return true;
  }
...
function evaluateRules(user, resource, action) {
  ...
  const permissions = rules.filter(
    (r) => r.actions.includes(action) && r.id !== "REQ_CONTEXT_CITY",
  );
