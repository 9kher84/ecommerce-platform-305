...
async function runTests() {
  ...
  try {
    ...
    if (
      ...
      invalidPostData.message.includes("العنوان مطلوب")
    ) {
    ...
    if (
      ...
      negativePriceData.message.includes("السعر الابتدائي")
    ) {
    ...
    const invalidOfferRes = await fetch(
      `${baseUrl}/posts/${dummyPostId}/offers`,
      {
        method: "POST",
