  ...
  try {
    ...
    if (res.status === 201) {
      ...
      if (data.user.role === "buyer") {
        ...
        console.error(
          `❌ FAIL: API created user with role "${data.user.role}". Security vulnerability exists!`,
        );
        process.exit(1);
      }
    } else if (
      ...
      (data.message.includes("role") ||
        JSON.stringify(data.errors).includes("role"))
    ) {
      console.log(
        '✅ PASS: API rejected "admin" role via Validation Middleware.',
      );
    } else {
    ...
    const setupContent = fs.readFileSync(
      path.join(__dirname, "../sequelize_setup.js"),
      "utf8",
    );

    if (setupContent.includes("force: true")) {
      console.error(
        '❌ FAIL: Found "force: true" in sequelize_setup.js. Data loss risk exists!',
      );
      process.exit(1);
    } else {
