...
describe('🚨 Live Fire Exercise: Honeypot & AI Shield', () => {
    ...
    test('1. Honeypot Attack mechanics trigger Kill Switch', async () => {
        ...
        const blockSpy = jest.spyOn(sovereignKillSwitch, 'blockIncomingConnections').mockResolvedValue(true);
        const persistSpy = jest.spyOn(sovereignKillSwitch, 'persistSystemState').mockResolvedValue(true);
