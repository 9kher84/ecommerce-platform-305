...
exec('npm audit --json', (error, stdout, stderr) => {
    try {
        ...
        if (totalHighCritical > 0) {
            ...
            console.error('   Access DENIED. Fix vulnerabilities using "npm audit fix" before deploying.');
            process.exit(1);
        } else {
        ...
        console.error('⚠️  Audit execution failed:', error ? error.message : 'Unknown');
        process.exit(1);
    }
});
