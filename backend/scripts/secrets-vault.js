...
const ensureProductionSecrets = () => {
  if (process.env.NODE_ENV === "production") {
    if (!process.env.VAULT_ADDR || !process.env.VAULT_TOKEN) {
      console.error(
        "❌ [CRITICAL ERROR] In production: VAULT_ADDR and VAULT_TOKEN must be defined",
      );
      console.error(
        "⛔ FATAL: Cannot start without Vault configuration in production",
      );
      process.exit(1);
    }
    console.log(
      `✅ [PRODUCTION] Connected to Vault: ${process.env.VAULT_ADDR}`,
    );
  } else {
    console.log(
      "⚠️  [DEVELOPMENT] Using .env - Vault will be required in production",
    );
  }
};
...
const loadSecrets = async () => {
  try {
    ...
    const secretPath =
      process.env.VAULT_SECRET_PATH || "secret/data/ecommerce/prod";
    ...
    try {
      ...
      console.warn(
        `⚠️  Could not connect to Vault at ${process.env.VAULT_ADDR || "localhost"}: ${vaultError.message}`,
      );
      ...
      if (process.env.NODE_ENV === "production") {
        ...
        console.error(
          "🚨 System cannot start without Vault in production mode",
        );
        process.exit(1);
      }
    ...
    if (missing.length > 0) {
      ...
      if (process.env.NODE_ENV === "production") {
        ...
        console.warn(
          "⚠️  WARNING: Missing secrets in development - application may not function correctly",
        );
      }
    } else {
    ...
    if (process.env.NODE_ENV === "production") {
      console.error(
        "⛔ FATAL: Cannot start in production without proper secret management",
      );
      process.exit(1);
    }
  }
