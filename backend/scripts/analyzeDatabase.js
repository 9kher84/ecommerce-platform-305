...
async function suggestIndexes() {
  ...
  const patterns = [
    ...
    { table: "Users", columns: ["role"], reason: "Role-based queries" },
    {
      ...
      reason: "User request filtering",
    },
    {
      table: "PurchaseRequests",
      columns: ["status", "createdAt"],
      reason: "Status and date filtering",
    },
    {
      table: "PurchaseRequests",
      columns: ["categoryId"],
    ...
    },
    {
      table: "PriceQuotes",
      columns: ["requestId"],
      reason: "Request-quote relationship",
    },
    {
      table: "PriceQuotes",
      columns: ["sellerId"],
      reason: "Seller quote queries",
    },
    {
      table: "AuditLogs",
      columns: ["createdAt"],
      reason: "Date-based cleanup",
    },
    {
      table: "AuditLogs",
      columns: ["userId"],
  ...
  try {
    ...
    patterns.forEach((pattern) => {
      ...
      if (!hasIndex) {
        ...
        console.log(
          `     SQL: CREATE INDEX idx_${pattern.table.toLowerCase()}_${pattern.columns.join("_").toLowerCase()} ON "${pattern.table}" (${pattern.columns.map((c) => `"${c}"`).join(", ")});`,
        );
      }
    });
