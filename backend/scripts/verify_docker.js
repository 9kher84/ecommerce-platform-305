...
console.log("\n📦 Checking Dockerfile:");
assert(
  dockerfile.includes("FROM node:18-alpine"),
  "Base image is node:18-alpine",
);
assert(dockerfile.includes("HEALTHCHECK"), "HEALTHCHECK instruction exists");
assert(
  dockerfile.includes("curl -f http://localhost:5000/api/health"),
  "Healthcheck targets correct endpoint",
);
assert(
  dockerfile.includes("npm ci --only=production"),
  "Clean production install enforced",
);
