...
async function migrate() {
  ...
  try {
    ...
    try {
      ...
      if (
        tableInfo.name &&
        (tableInfo.name.type.includes("VAR") ||
          tableInfo.name.type.includes("CHAR") ||
          tableInfo.name.type.includes("TEXT"))
      ) {
        needMigration = true;
      }
    } catch (e) {}
    ...
    if (needMigration) {
      console.log(
        "Migrating name column (Rename -> Create -> Move Data -> Drop Old)...",
      );
      try {
        ...
        console.error(
          "Error during name migration (might have partially failed or already run):",
          err.message,
        );
