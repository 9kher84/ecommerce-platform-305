...
const loadSecrets = async () => {
    try {
        ...
        if (fs.existsSync(vaultPath)) {
            ...
            console.warn('⚠️ Vault secrets file not found. Relying on existing environment variables.');
        }
        ...
        if (missing.length > 0) {
            console.warn(`⚠️  WARNING: Critical secrets missing from environment: ${missing.join(', ')}`);
        } else {
