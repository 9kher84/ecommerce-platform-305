...
async function runTest() {
  try {
    ...
    if (loginRes.data.user.role !== "seller") {
      console.error(
        "❌ FAIL: User is not a seller. Current role:",
    ...
    try {
      ...
      if (productRes.status === 200 || productRes.status === 201) {
        ...
        console.log(
          "❌ FAIL: Seller was forbidden (403). Middleware not working correctly.",
        );
        console.log("   Response:", productRes.data);
      } else {
    ...
    try {
      const buyerProductRes = await client.post(
        "/products",
        {
          name: "Test Product",
          description: "Test Description",
      ...
      if (buyerProductRes.status === 403) {
        ...
        if (
          buyerProductRes.data.message &&
          buyerProductRes.data.message.includes("Only sellers")
        ) {
          console.log("   ✅ Correct error message confirmed");
        }
      } else {
        console.log(
          "❌ FAIL: Buyer was NOT forbidden. Status:",
          buyerProductRes.status,
        );
