...
exports.cacheBulkData = async (data) => {
    ...
    if (!isRedisAvailable() || !redis) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            console.warn('⚠️ Redis not available. Falling back to UNSAFE memory cache for DEV.');
            global.unsafeDevCache = global.unsafeDevCache || new Map();
            global.unsafeDevCache.set(token, { data, expires: Date.now() + CACHE_TTL_SECONDS * 1000 });
