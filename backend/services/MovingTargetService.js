...
class MovingTargetDefense {
  ...
  rotateHeaders() {
    ...
    logger.info(
      `🔄 Moving Target Defense: Header Rotated to [${this.currentHeader}]`,
    );
  }
  ...
  getMiddleware() {
    return (req, res, next) => {
      ...
      if (!headerValue && !req.path.startsWith("/public")) {
        logger.warn(
          `🛡️ Moving Target Defense: Request dropped. Missing header ${this.currentHeader}`,
        );
        return res
          .status(418)
          .json({ error: "خطأ سيادي: البروتوكول غير متزامن" });
      }
