...
exports.sanitizeNegotiationUpdates = async (userId, updates) => {
  ...
  const isNegotiationUpdate =
    ...
    (updates.productTier &&
      ["smart", "ai_assisted"].includes(updates.productTier));
  ...
  if (canNegotiate) {
    ...
    if (sanitized.autoNegotiationEnabled !== undefined)
      delete sanitized.autoNegotiationEnabled;
    if (sanitized.minAcceptablePrice !== undefined)
      delete sanitized.minAcceptablePrice;
    if (sanitized.negotiationStrategy !== undefined)
      delete sanitized.negotiationStrategy;
    // Downgrade tier if attempting higher tier without eligibility
    if (
      sanitized.productTier &&
      ["smart", "ai_assisted"].includes(sanitized.productTier)
    ) {
