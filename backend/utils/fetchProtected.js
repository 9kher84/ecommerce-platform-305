...
function validateUrl(urlString) {
  try {
    ...
    if (!["http:", "https:"].includes(parsedUrl.protocol)) {
      throw new Error(
        `Protocol ${parsedUrl.protocol} is not allowed. Only HTTP/HTTPS are permitted.`,
      );
    }
    ...
    if (
      localhostPatterns.some((pattern) => hostname.toLowerCase() === pattern)
    ) {
    ...
    if (privateIPPatterns.some((pattern) => pattern.test(hostname))) {
      throw new Error(
        "Access to private IP addresses is forbidden (SSRF protection)",
      );
    }
...
async function fetchImageProtected(imageUrl) {
  ...
  console.log(
    `✅ [SSRF Protection] Image fetched: ${imageUrl} (${response.data.length} bytes)`,
  );
