...
async function run() {
  ...
  // 5b. NEGOTIATE QUOTE
  console.log('\n[5b] Negotiating Quote...');
  const negotiateQuote = await request('POST', `/quotes/${quoteId}/negotiate`, {
    price: 13
  }, buyerToken);
  console.log(`Negotiate Quote Status: ${negotiateQuote.status}`, JSON.stringify(negotiateQuote.data).substring(0, 300));
  if (negotiateQuote.status !== 200) return console.log('❌ Failed to negotiate quote');

  // 5c. SELLER RESPONDS TO NEGOTIATION
  console.log('\n[5c] Seller Responds to Negotiation...');
  const respondQuote = await request('POST', `/quotes/${quoteId}/respond`, {
    accept: true,
    newPrice: 13
  }, sellerToken);
  console.log(`Respond Quote Status: ${respondQuote.status}`, JSON.stringify(respondQuote.data).substring(0, 300));
  if (respondQuote.status !== 200) return console.log('❌ Failed to respond to quote');

  // 6. ACCEPT QUOTE
  console.log('\n[6] Accepting Quote...');
  const acceptQuote = await request('POST', `/quotes/${quoteId}/accept`, {
    decision_reason: 'Good price'
  }, buyerToken);
  console.log(`Accept Quote Status: ${acceptQuote.status}`, JSON.stringify(acceptQuote.data).substring(0, 500));
  ...
  if (acceptQuote.status === 201 || acceptQuote.status === 200) {
    ...
    // 7. PAYMENT SYSTEM ENABLED IN DB ALREADY
    console.log('\n[7] Payment System assumed enabled in DB');
