...
const aiOutputSanitizer = (req, res, next) => {
        ...
        if (typeof data === 'string') {
            ...
            SENSITIVE_PATTERNS.forEach(pattern => {
                if (pattern.test(sanitized)) {
                    unauthorized = true;
                    sanitized = sanitized.replace(pattern, '[REDACTED_BY_SOVEREIGN_PROTOCOL]');
                }
            });

            if (unauthorized) {
                logger.warn(`🛡️ AI Output Sanitizer blocked sensitive data leak in request ${req.id}`);
    ...
    res.send = function (body) {
        // Only sanitize if this looks like an AI/LLM response or generic API
