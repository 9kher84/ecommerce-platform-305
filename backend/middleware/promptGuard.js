const promptGuard = (req, res, next) => {
  ...
  if (foundInBody) {
    console.warn(
      `[Security] Blocked request containing forbidden keyword in body: ${foundInBody}`,
    );
  ...
  if (foundInQuery) {
    console.warn(
      `[Security] Blocked request containing forbidden keyword in query: ${foundInQuery}`,
    );
    return res.status(403).json({ error: "Malicious content detected." });
  }
  ...
  if (foundInParams) {
    console.warn(
      `[Security] Blocked request containing forbidden keyword in params: ${foundInParams}`,
    );
    return res.status(403).json({ error: "Malicious content detected." });
  }
