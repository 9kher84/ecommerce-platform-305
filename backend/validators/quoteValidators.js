...
const submitQuoteSchema = Joi.object({
  ...
  fixedPrice: Joi.number()
    .positive()
    .when("priceType", { is: "fixed", then: Joi.required() }),
  priceRangeMin: Joi.number()
    .positive()
    .when("priceType", { is: "flexible", then: Joi.required() }),
  priceRangeMax: Joi.number()
    .positive()
    .greater(Joi.ref("priceRangeMin"))
    .when("priceType", { is: "flexible", then: Joi.required() }),
  flexibilityReason: Joi.string().when("priceType", {
    is: "flexible",
    then: Joi.required(),
  }),
