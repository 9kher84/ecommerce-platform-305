...
const initiatePaymentSchema = Joi.object({
  dealId: Joi.number().integer().positive().required().messages({
  ...
  currency: Joi.string().length(3).uppercase().default("SAR").messages({
    "string.length": "Currency must be 3 characters (ISO 4217)",
    "string.uppercase": "Currency must be uppercase",
  }),

  paymentGateway: Joi.string()
    .valid("mada", "stc_pay", "stcpay", "apple_pay", "test")
    .required()
    .messages({
      "any.only":
        "Invalid payment gateway. Supported: mada, stc_pay, apple_pay",
...
const processPaymentSchema = Joi.object({
  gatewayResponse: Joi.object().required().messages({
...
const savePaymentMethodSchema = Joi.object({
  type: Joi.string()
    .valid("card", "wallet", "bank_account")
    .required()
    .messages({
  ...
  provider: Joi.string()
    .valid("mada", "stc_pay", "apple_pay", "visa", "mastercard")
    .required()
    .messages({
  ...
  cardData: Joi.object({
    lastFour: Joi.string().length(4).pattern(/^\d+$/).required().messages({
...
const webhookSchema = Joi.object({
  ...
  status: Joi.string()
    .valid("completed", "failed", "pending", "refunded")
    .required(),
