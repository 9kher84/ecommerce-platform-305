...
async function testAuthFlow() {
  ...
  try {
    ...
    if (setCookie) {
      ...
      if (
        cookieStr.includes("HttpOnly") &&
        cookieStr.includes("Secure") &&
        cookieStr.includes("SameSite=Strict")
      ) {
        console.log(
          "   ✅ SUCCESS: Cookie attributes correct (HttpOnly; Secure; SameSite=Strict).",
        );
      } else {
        console.warn(
          "   ⚠️ WARNING: Check cookie attributes manually:",
          cookieStr,
        );
      }
    } else {
    ...
    try {
      ...
      console.log(
        "   ✅ SUCCESS: Accessed protected route. User:",
        meRes.data.data.email,
      );
    } catch (meErr) {
      console.error(
        "   ❌ FAILURE: Could not access protected route:",
        meErr.response ? meErr.response.data : meErr.message,
      );
    }
  } catch (error) {
    console.error(
