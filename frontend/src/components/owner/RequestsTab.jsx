...
const RequestsTab = () => {
  ...
  const handleConfirmForce = async () => {
    ...
    try {
      await ownerService.forceRequestTransition(
        selectedReq.id,
        forceState,
        forceReason,
      );
              ...
              {requests.map((req) => (
                ...
                >
                  ...
                  <td className="p-4 text-right space-x-2">
                    <button
                      onClick={() =>
                        handleInspectPolicy(req.id, req.userId, req.user?.name)
                      }
                    ...
                    <button
                      onClick={() => {
                        setSelectedReq(req);
                        setForceState(req.status);
                        setForcePreviewTrace(null);
                      }}
      ...
      {viewingTrace && viewTraceData && (
        <div className="fixed inset-0 z-[100]">
          ...
          <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-indigo-600 px-4 py-1 rounded text-xs font-bold text-white shadow-xl z-[110]">
            SIMULATING POLICY FOR ACTOR:{" "}
            {viewTraceData.simulatedActorName || "USER"}
          </div>
        </div>
      )}
          ...
          <div className="bg-neutral-900 border border-red-900/50 rounded-xl w-full max-w-5xl shadow-2xl flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-neutral-800 bg-neutral-950 rounded-t-xl">
              ...
              <p className="text-xs text-neutral-500 font-mono">
                TARGET: {selectedReq.id} | CURRENT: {selectedReq.status}
              </p>
            </div>
            ...
            {forceState === "DONE" ? (
              <div className="flex-1 flex flex-col items-center justify-center p-12 text-center bg-black/50">
                ...
                <p className="text-neutral-400 max-w-md mb-8">
                  The request state has been forced. This action is irreversible
                  and has been logged in the Sovereign Audit Trail.
                </p>
                <button
                  onClick={() => {
                    setSelectedReq(null);
                ...
                <div className="w-1/3 p-6 border-r border-neutral-800 space-y-6 bg-neutral-950">
                  ...
                  <div className="pt-6 border-t border-neutral-800">
                    <button
                      ...
                      disabled={
                        !forceState || forceState === selectedReq.status
                      }
                ...
                <div className="w-2/3 bg-black flex flex-col relative">
                  {!forcePreviewTrace ? (
                    <div className="flex-1 flex items-center justify-center text-neutral-600 italic p-10 text-center">
                      Select Target State and Click "Preview Impact" to verify
                      consequences before Forcing.
                    </div>
                  ) : (
                        ...
                        <ul className="text-xs space-y-1.5 text-neutral-300 list-disc pl-4">
                          <li>
                            ...
                            <strong className="text-red-400">
                              {forcePreviewTrace.decision || "DENY"}
                            </strong>{" "}
                            (System would block this).
                          </li>
                          <li>
                            Forced Outcome: Request will immediately transition
                            to{" "}
                            <strong className="text-white">{forceState}</strong>
                            .
                          </li>
                          <li>
                            Traceability: This manual intervention is being{" "}
