...
const OfferRow = ({ offer, onAcceptOffer }) => {
  ...
  return (
    <tr className="border-b hover:bg-gray-50">
      ...
      <td className="p-3">
        <span
          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${statusClasses[offer.status] || "bg-gray-100 text-gray-800"}`}
        >
      ...
      <td className="p-3 text-center">
        ...
        {!canAccept && offer.status === "accepted" && (
          <span className="text-xs text-green-500">تمت كصفقة</span>
        )}
        {!canAccept && offer.status === "rejected" && (
...
const PostManagement = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        showNotification(
          err.response?.data?.message || "فشل في تحميل بيانات المنشور.",
          "error",
        );
      } finally {
        setLoading(false);
  ...
  const handleAcceptOffer = async (offerId, amount) => {
    if (
      !window.confirm(
        `هل أنت متأكد من قبول العرض بقيمة ${parseFloat(amount).toFixed(2)} ريال وإغلاق المزايدة؟ هذا الإجراء لا يمكن التراجع عنه.`,
      )
    )
      return;
    ...
    try {
      ...
      showNotification(
        "✅ تم قبول العرض بنجاح. تم إنشاء صفقة (Deal) وإغلاق المنشور.",
        "success",
      );
    ...
    <div className="p-4 md:p-8 space-y-8">
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg border-t-4 border-indigo-600">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <p>
            <strong>سعر البدء:</strong>{" "}
            <span className="text-blue-600">
              {parseFloat(post.startingPrice).toFixed(2)} ريال
            </span>
          </p>
          <p>
            <strong>أعلى عرض حالي:</strong>{" "}
            <span className="text-green-600">
              {parseFloat(
                post.currentHighestOffer || post.startingPrice,
              ).toFixed(2)}{" "}
              ريال
            </span>
          </p>
          <p>
            ...
            {new Date(post.expiryDate).toLocaleString("ar-EG")}
          </p>
          <p>
            <strong>الحالة:</strong>
            <span
              className={`font-semibold ml-2 p-1 rounded ${post.status === "active" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
            >
              {post.status === "active"
                ? "نشط"
                : post.status === "sold"
      ...
      {post.status !== "active" && (
        <div
          className={`p-4 rounded-md ${post.status === "sold" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"} font-medium`}
        >
          ⚠️ هذا المنشور **{post.status === "sold" ? "مُباع" : "انتهت مدته"}.**
          لا يمكن قبول أي عروض جديدة.
        </div>
      )}
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="p-3 text-center text-xs font-medium text-gray-500 uppercase">
                    الإجراء
                  </th>
                </tr>
              </thead>
