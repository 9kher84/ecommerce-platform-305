...
const RegisterComponent = ({ onNavigate, onShowMessage }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // نجاح التسجيل
      onShowMessage(
        `تم إنشاء الحساب بنجاح، مرحباً بك يا ${data.data.user.name}. يرجى تسجيل الدخول.`,
        "success",
      );
    ...
    if (user && user.role) {
      if (user.role === "admin" || user.role === "super_admin") {
        items.push(
          ...
          {
            id: PAGES.ADMIN_USERS,
            label: "إدارة المستخدمين",
            icon: Users,
            roles: ["admin", "super_admin"],
          },
          {
            id: PAGES.ADMIN_TRANSACTIONS,
            label: "سجل الصفقات",
            icon: DollarSign,
            roles: ["admin", "super_admin"],
          },
              ...
              user ? (
                <div className="relative">
                  ...
                  >
                    ...
                    <span>
                      {user.name || "المستخدم"} ({user.role})
                    </span>
                    <ChevronDown
                      className={`w-4 h-4 transition-transform ${isMenuOpen ? "rotate-180" : "rotate-0"}`}
                    />
                  </button>
                  {isMenuOpen && (
...
const SimpleMessageModal = ({ message, type = "info", onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50">
      <div
        className={`p-6 rounded-lg shadow-2xl max-w-sm w-full border-t-4 ${colorClasses[type]}`}
      >
        ...
        {type === "error" && (
          <p className="text-xs mt-2">يرجى المحاولة مرة أخرى.</p>
        )}
      </div>
    </div>
  );
...
const App = () => {
    ...
    switch (currentPage) {
      case PAGES.DASHBOARD:
        ...
        return (
          <PlaceholderPage
            title="لوحة التحكم"
            description={`أهلاً بك يا ${user.name}، دورك هو: ${user.role}. سيتم بناء لوحة التحكم الديناميكية هنا.`}
          />
        );
      case PAGES.PROFILE:
        ...
        return (
          <PlaceholderPage
            title="الملف الشخصي"
            description="سيتم بناء واجهة لتحديث الملف الشخصي هنا."
          />
        );
      case PAGES.POSTS:
        ...
        return (
          <PlaceholderPage
            ...
            description="سيتم بناء واجهة إنشاء/إدارة المنشورات والمزايدة هنا."
          />
        );
        ...
        if (user.role === "admin" || user.role === "super_admin") {
          return (
            <PlaceholderPage
              ...
              description="سيتم بناء واجهة إضافة/تعديل التصنيفات هنا."
            />
          );
        }
        ...
        if (user.role === "admin" || user.role === "super_admin") {
          return (
            <PlaceholderPage
              ...
              description="سيتم بناء واجهة حظر/تعديل أدوار المستخدمين هنا."
            />
          );
        }
        return <PermissionDenied />;
      case PAGES.ADMIN_TRANSACTIONS:
        ...
        if (user.role === "admin" || user.role === "super_admin") {
          return (
            <PlaceholderPage
              title="سجل الصفقات"
              description="سيتم بناء واجهة تقارير الصفقات هنا."
            />
          );
        }
        return <PermissionDenied />;
      default:
        return (
          <PlaceholderPage
            ...
            description="عُد إلى لوحة التحكم."
            actionLabel="لوحة التحكم"
            action={() => setCurrentPage(PAGES.DASHBOARD)}
          />
        );
    }
  };
  ...
  return (
    <div
      dir="rtl"
      className="min-h-screen bg-gray-50 flex flex-col font-sans antialiased"
    >
      <style>{`
...
const PlaceholderPage = ({ title, description, actionLabel, action }) => (
  <div className="p-10 text-center bg-white border border-gray-100 rounded-xl shadow-lg mt-8">
    ...
    <p className="text-sm text-gray-400 mb-8">
      <span className="font-bold">ملاحظة:</span> هذه واجهة وهمية، سيتم استبدالها
      بمكون React الكامل في الخطوات التالية.
    </p>
    {action && (
      <button
        onClick={action}
