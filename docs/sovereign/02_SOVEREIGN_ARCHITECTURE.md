...
- **Role**: The "Legislative" branch. Defines what _should_ happen.

### 2. Owner Controller

- **Location**: `backend/controllers/ownerController.js`
- **Function**: The "Executive" branch. Can execute `override*` methods.
- **Constraint**: MUST generate a `TraceSnapshot` of the _policy violation_ before executing the _state mutation_.
